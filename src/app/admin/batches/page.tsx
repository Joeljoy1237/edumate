      ...
      <div className="w-full">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Group students for academic operations and management.
          </p>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {batches.map((batch) => (
                  <tr key={batch.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(batch.status || "active")}`}
                      >
                        {(batch.status || "active").charAt(0).toUpperCase() +
                          (batch.status || "active").slice(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-3">
                      ...
                      <Link href={`/admin/batches/edit/${batch.id}`}>
                        <button className="text-blue-600 hover:text-blue-900 hover:bg-blue-50 p-1.5 rounded transition-colors">
                          Edit
                        </button>
                      </Link>
                      <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl p-6 w-[400px] animate-in zoom-in-95 duration-200">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Promote <strong>{selectedBatch.name}</strong> to the next
                semester.
              </p>
