...
export default function CollegePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const profileSnap = await getDoc(
          doc(db, "settings", "college_profile"),
        );
    ...
    const unsubAcc = onSnapshot(
      collection(db, "college_accreditation"),
      (snap) => {
        setAccreditation(
          snap.docs.map((d) => ({ id: d.id, ...d.data() }) as Accreditation),
        );
      },
    );
    const unsubYears = onSnapshot(collection(db, "academic_years"), (snap) => {
      setAcademicYears(
        snap.docs.map((d) => ({ id: d.id, ...d.data() }) as AcademicYear),
      );
    });
      ...
      <div className="w-full">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage institution profile, accreditation, and academic calendars.
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() =>
                setEditingSection(
                  editingSection === "profile" ? null : "profile",
                )
              }
        ...
        <div className="bg-white rounded-lg border border-gray-200 mb-8 overflow-hidden">
          <div className="p-6 border-b flex justify-between items-center">
            ...
            <button
              onClick={() =>
                setEditingSection(
                  editingSection === "accreditation" ? null : "accreditation",
                )
              }
          ...
          {editingSection === "accreditation" && (
            <div className="p-6 bg-blue-50 grid grid-cols-1 md:grid-cols-5 gap-4 items-end border-b">
              <input
                type="text"
                placeholder="Body (e.g. NAAC)"
                className="border p-2 rounded"
                value={accForm.body || ""}
                onChange={(e) =>
                  setAccForm({ ...accForm, body: e.target.value })
                }
              />
              <input
                type="text"
                ...
                value={accForm.grade || ""}
                onChange={(e) =>
                  setAccForm({ ...accForm, grade: e.target.value })
                }
              />
              <input
                type="text"
                ...
                value={accForm.year || ""}
                onChange={(e) =>
                  setAccForm({ ...accForm, year: e.target.value })
                }
              />
              <input
                type="text"
                ...
                className="border p-2 rounded"
                value={accForm.validity || ""}
                onChange={(e) =>
                  setAccForm({ ...accForm, validity: e.target.value })
                }
              />
              <button
        ...
        <div className="bg-white rounded-lg border border-gray-200 mb-8 overflow-hidden">
          <div className="p-6 border-b flex justify-between items-center">
            ...
            <button
              onClick={() =>
                setEditingSection(editingSection === "years" ? null : "years")
              }
              className="px-4 py-2 bg-blue-600 text-white rounded text-sm"
            >
          ...
          {editingSection === "years" && (
            <div className="p-6 bg-blue-50 grid grid-cols-1 md:grid-cols-6 gap-4 items-end border-b">
              ...
              <input
                ...
                onChange={(e) =>
                  setYearForm({ ...yearForm, semesters: e.target.value })
                }
              />
              <button
            ...
            <tbody className="divide-y divide-gray-200">
              {academicYears.map((y) => (
                <tr key={y.id}>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded text-xs ${getStatusColor(y.status)}`}
                    >
                  ...
                  <td className="px-6 py-4">
                    <button
                      onClick={() => toggleYearStatus(y)}
                      className="text-blue-600 hover:text-blue-900 text-xs font-bold mr-2"
                    >
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200 mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() =>
                setEditingSection(
                  editingSection === "settings" ? null : "settings",
                )
              }
              className="text-blue-600 hover:text-blue-800 font-medium text-sm"
            >
          ...
          {editingSection === "settings" ? (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      maxCreditHours: parseInt(e.target.value),
                    })
                  }
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 text-sm">
              <div>
                <span className="text-gray-500 block">Max Credits</span>{" "}
                <span className="font-medium text-lg">
                  {settings.maxCreditHours}
                </span>
              </div>
              <div>
                <span className="text-gray-500 block">Min CGPA</span>{" "}
                <span className="font-medium">{settings.minCGPA}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-gray-500 block">Grading Scale</span>{" "}
                <span className="font-medium">{settings.gradingScale}</span>
              </div>
              <div>
