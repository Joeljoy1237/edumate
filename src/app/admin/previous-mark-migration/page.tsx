...
export default function PreviousMarkMigrationPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setLogs(
        snap.docs.map((d) => ({ id: d.id, ...d.data() }) as MigrationLog),
      );
      setLoading(false);
    });
    return () => unsub();
  ...
  const validateData = async () => {
    ...
    try {
      ...
      const validRegNos = new Set(
        studentsSnap.docs.map((d) => d.data().registerNumber),
      );
      const validSubCodes = new Set(
        subjectsSnap.docs.map((d) => d.data().code),
      );

      const validated = parsedData.map((record) => {
        let error = "";
        if (!validRegNos.has(record.regNumber))
          error += `Student ${record.regNumber} not found. `;
        if (!validSubCodes.has(record.subjectCode))
          error += `Subject ${record.subjectCode} not found. `;
  ...
  const processImport = async () => {
    if (parsedData.some((r) => r.status === "Invalid")) {
      if (!confirm("There are invalid records. Import only VALID records?"))
        return;
    }
  ...
  const handleRollback = async (targetImportId: string) => {
    if (
      !confirm(
        "Rollback this import? This will delete all records associated with it.",
      )
    )
      return;
    ...
    try {
      const q = query(
        collection(db, "previous_marks"),
        where("importId", "==", targetImportId),
      );
      ...
      <div className="w-full">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            Previous Mark Migration
          </h1>
          <p className="text-gray-600 mt-2">
            Bulk import historical marks via Excel/CSV.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 mb-8 p-6">
          ...
          <p className="text-xs text-gray-400 mt-2">
            Required Columns: RegNumber, SubjectCode, Marks, Semester
          </p>
        </div>
                  ...
                  {parsedData.map((record, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded text-xs ${record.status === "Valid" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                        >
                          {record.status}
                        </span>
                      </td>
