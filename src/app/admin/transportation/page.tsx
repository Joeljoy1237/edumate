...
export default function TransportationPage() {
  ...
  useEffect(() => {
    const unsubRoutes = onSnapshot(
      collection(db, "transport_routes"),
      (snap) => {
        setRoutes(
          snap.docs.map((d) => ({ id: d.id, ...d.data() }) as TransportRoute),
        );
      },
    );
    ...
    const unsubAlloc = onSnapshot(
      collection(db, "student_transport"),
      (snap) => {
        setAllocations(
          snap.docs.map(
            (d) => ({ id: d.id, ...d.data() }) as TransportAllocation,
          ),
        );
      },
    );
  ...
  const handleAddRoute = async () => {
    try {
      const stopsArray = routeForm.stopsString
        ? routeForm.stopsString.split(",").map((s: string) => s.trim())
        : [];
      ...
      <div className="w-full">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage bus routes, vehicles, and student transport subscriptions.
          </p>
        </div>
                  ...
                  {routes.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() => handleDelete("transport_routes", r.id)}
                  ...
                  {vehicles.map((v) => (
                    <tr key={v.id}>
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() =>
                            handleDelete("transport_vehicles", v.id)
                          }
                          className="text-red-500 hover:text-red-700 text-sm font-bold"
                        >
                          Delete
                  ...
                  {allocations.map((a) => (
                    <tr key={a.id}>
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={() =>
                            handleDelete("student_transport", a.id)
                          }
                          className="text-red-500 hover:text-red-700 text-sm font-bold"
                        >
                          Remove
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <input
              ...
              onChange={(e) =>
                setRouteForm({ ...routeForm, routeNumber: e.target.value })
              }
            />
            <input
              type="text"
            ...
            <input
              ...
              onChange={(e) =>
                setRouteForm({ ...routeForm, stopsString: e.target.value })
              }
            />
            <input
              type="text"
              ...
              onChange={(e) =>
                setRouteForm({ ...routeForm, driver: e.target.value })
              }
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <select
              ...
              onChange={(e) =>
                setAllocForm({ ...allocForm, studentId: e.target.value })
              }
            >
            ...
            <select
              ...
              onChange={(e) =>
                setAllocForm({ ...allocForm, routeId: e.target.value })
              }
            >
            ...
            <input
              ...
              className="w-full border p-2 mb-2 rounded"
              value={allocForm.stop}
              onChange={(e) =>
                setAllocForm({ ...allocForm, stop: e.target.value })
              }
            />
            <input
              type="number"
              ...
              className="w-full border p-2 mb-4 rounded"
              value={allocForm.fees}
              onChange={(e) =>
                setAllocForm({ ...allocForm, fees: e.target.value })
              }
            />
