          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
            <div className="min-w-full">
              <div className="flex bg-gray-50 border-b border-gray-200">
                <div className="p-4 w-32 border-r border-gray-200">
                  <Skeleton className="h-4 w-20" />
                </div>
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="p-4 flex-1 text-center min-w-[140px]">
              ...
              {[1, 2, 3, 4, 5].map((row) => (
                <div key={row} className="flex border-b border-gray-100">
                  <div className="p-4 w-32 border-r border-gray-200 bg-gray-50 flex items-center">
                    <Skeleton className="h-4 w-24" />
                  </div>
                  {[1, 2, 3, 4, 5, 6].map((col) => (
                    <div
                      key={col}
                      className="p-2 flex-1 border-l border-gray-100 min-w-[140px] h-24"
                    >
                      {/* Randomly sparsely populate to look realistic */}
                      {(row + col) % 3 === 0 && (
                        <div className="h-full bg-gray-100 rounded border border-gray-200 p-2">
        ...
        <div className="mb-8 flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Design and manage weekly schedules for batches.
            </p>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-32 border-r">
                  Time Slot
                </th>
                {DAYS.map((day) => (
                  <th
                    key={day}
                    className="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider min-w-[140px]"
                  >
                    {day}
                  </th>
                ))}
                ...
                <tr key={time}>
                  ...
                  {DAYS.map((day) => {
                    ...
                    return (
                      <td
                        key={day + time}
                        className="px-2 py-2 text-center border-l border-gray-100 relative group h-24 align-top"
                      >
                        {entry ? (
                          <div
                            className={`p-2 rounded-lg text-xs h-full flex flex-col justify-between ${entry.status === "conflict" ? "bg-red-50 border border-red-200" : "bg-blue-50 border border-blue-100"}`}
                          >
                            <div>
